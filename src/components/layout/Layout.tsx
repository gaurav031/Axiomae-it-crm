...
import { LayoutDashboard, Users, PhoneCall, Mail, CheckSquare, MessageSquare, Shield, LogOut, Upload, UserPlus, CalendarClock, Ban, CheckCircle, BarChart, Building2, FileText, User } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-sm">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10">
        <div className="h-16 flex items-center px-6 border-b border-gray-200 justify-center">
          <img src={logoFull} alt="Axiomae IT Logo" className="h-10 w-auto object-contain drop-shadow-sm" />
          <h1 className="text-xl font-extrabold text-primary tracking-tight">CRM</h1>
        </div>
        <nav className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
          ...
          <div className="space-y-1">
            <div className="px-3 text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Account</div>
            <Link to="/profile" className={`flex items-center px-3 py-2 rounded-md transition-colors ${isActive('/profile') ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-50'}`}>
